...
void cloudUp(int value) {
    ...
    cloudP -= cloudS;
	glutPostRedisplay();
	glutTimerFunc(200, cloudUp, 0);
}

GLfloat whiteFlowerpos = 0.0;
GLfloat rainAutumnTime = 0.0;
GLfloat lateAutumnTime = 0.0;
GLfloat treeP1=0.0;
GLfloat treeP2=0.0;
GLfloat treeP3=0.0;
GLfloat treeP4=0.0;
...
///////////white flower
           ...
           glVertex2f(-.43+whiteFlowerpos,-.55);
           glEnd();
...
////////flower down
    ...
    //glutTimerFunc(1000, display_summer_rain, 0);
    Idle();
    glFlush();
}
...
///////////////////////////////////////RAIN_AUTUMN//////////////////////////////////////////////////////////
void rain_autumn() {
    rainAutumnTime += 0.008;
    if(rainAutumnTime < 5)
        whiteFlowerpos += 0.0001;
    else if(rainAutumnTime < 15)
        whiteFlowerpos -= 0.0001;
    else if(rainAutumnTime < 20)
        whiteFlowerpos += 0.0001;
    else
        rainAutumnTime = whiteFlowerpos = 0;
...
////////////////////tree leaf
        //right big///4
//1
        if(lateAutumnTime < 15)
            lateAutumnTime += 0.001;
        if(lateAutumnTime > 1  && lateAutumnTime < 15)
            treeP3 -= 0.0002;
	    glPushMatrix();
	    ...
	    //right small////1
        if(lateAutumnTime > 2.5  && lateAutumnTime < 15)
            treeP2 -= 0.00019;
	    glPushMatrix();
	    ...
	    glLoadIdentity();
	      ...
	      //left small///
        if(lateAutumnTime > 2  && lateAutumnTime < 15)
            treeP1 -= 0.0002;
	      glPushMatrix();
	   ...
	   //left big/////3
        if(lateAutumnTime > 3.5  && lateAutumnTime < 15)
            treeP4 -= 0.00023;
	   glPushMatrix();
 ...
 //mid////////2
        glPushMatrix();
        if(lateAutumnTime > 3.1  && lateAutumnTime < 15)
            treeP5 -= 0.0002;
...
void handleKeypress(unsigned char key, int x, int y)
{
	...
	{
	    case '1':
            glutTimerFunc(1, display_summer, 0);
            break;
        case '2':
            rainSpeed = 0.2f;
            glutTimerFunc(1,display_rainyDay,0);
            break;
        case '3':
            glutTimerFunc(1,display_autumn,0);
            break;
	    case '4':
	        whiteFlowerpos = rainAutumnTime = 0.0;
            glutTimerFunc(1, display_rain_autumn, 0);
            break;
        case '5':
            treeP1 = treeP2 = treeP3 = treeP4 = treeP5 = lateAutumnTime = 0;
            glutTimerFunc(1,display_lateAutumn,0);
            break;
	    case '6':
            glutTimerFunc(1, display_winter, 0);
            break;
        case '7':
            glutTimerFunc(1,display_spring,0);
            break;
        case 'r':
            rainSpeed=0.2f;
            break;
